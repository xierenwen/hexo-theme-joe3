...
const commonContext = {
	...
	/* 初始化评论主题 */
	initCommentTheme() {
		const bgaOpts = (window.Joe && Joe.bloggerGenerateAvatarOpts) || {};
		if (
			...
			if (typeof Joe.replaceAllTextAvatarImage === "function") {
	...
	initMusic() {
		...
		if (window.__joeAPlayer) return;
		if (typeof APlayer === "undefined") {
			console.warn("[joe3] APlayer is not loaded");
			return;
		}
		const container = document.getElementById("global-aplayer");
		if (!container) return;

		const buildMusicUrl = (apiBase) => {
			const base = String(apiBase || "").replace(/\?$/, "");
			const joiner = base.indexOf("?") === -1 ? "?" : "&";
			return (
				`${base}${joiner}server=${encodeURIComponent(ThemeConfig.music_source || "netease")}` +
				`&type=${encodeURIComponent(ThemeConfig.music_player_type || "playlist")}` +
				`&id=${encodeURIComponent(ThemeConfig.music_list_id || "")}`
			);
		};

		const rewriteMediaUrl = (mediaUrl) => {
			if (!mediaUrl) return mediaUrl;
			try {
				const parsed = new URL(mediaUrl, window.location.origin);
				if (
					/injahow\.cn|i-meto\.com/i.test(parsed.hostname) &&
					parsed.searchParams.get("type") &&
					parsed.searchParams.get("id")
				) {
					return `/api/meting${parsed.search}`;
				}
			} catch (e) {}
			return mediaUrl;
		};

		const normalizeAudio = (list) => {
			if (!Array.isArray(list)) return [];
			return list
				.map((item) => ({
					name: item.name || item.title || "未知歌曲",
					artist: item.artist || item.author || "未知艺术家",
					url: rewriteMediaUrl(item.url || ""),
					cover: rewriteMediaUrl(item.cover || item.pic || ""),
					lrc: rewriteMediaUrl(item.lrc || ""),
				}))
				.filter((item) => !!item.url);
		};

		const loadPlaylist = (apiBase) =>
			fetch(buildMusicUrl(apiBase), { credentials: "same-origin", mode: "cors" }).then(
				(response) => {
		...
		const primaryApi = ThemeConfig.music_api || "/api/meting";
		const fallbackApi = "https://api.injahow.cn/meting/";

		loadPlaylist(primaryApi)
			.catch((err) => {
				if (primaryApi === fallbackApi) throw err;
				console.warn("[joe3] music primary api failed, fallback", err);
				return loadPlaylist(fallbackApi);
			})
			.then((res) => {
				if (window.__joeAPlayer) return;
				const audio = normalizeAudio(res);
				if (!audio.length) throw new Error("music api returned empty playlist");
				window.__joeAPlayer = new APlayer({
					container,
					fixed: true,
					lrcType: 3,
					...
					loop: ThemeConfig.music_loop_play || "all",
					listFolded: true,
					audio,
				});
			})
			.catch((err) => {
				console.warn("[joe3] initMusic failed", err);
			});
	},
	/* 渲染PDF */
	initPDF() {
	...
	back2Top() {
		...
		$(document)
			.off("scroll.joeBack2top")
			.on("scroll.joeBack2top", Utils.throttle(handleScroll, 120));
		$el.off("click.joeBack2top").on("click.joeBack2top", function (e) {
			e.stopPropagation();
			$("html,body").animate(
				{
					scrollTop: 0,
